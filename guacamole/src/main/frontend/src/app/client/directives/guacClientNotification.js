    ...
    directive.controller = [
      ...
      function guacClientNotificationController($scope, $injector, $element) {
        ...
        const notifyConnectionState = function notifyConnectionState(connectionState) {
          ...
          //   let actions;
          //   if (NAVIGATE_HOME_ACTION) actions = [NAVIGATE_HOME_ACTION, RECONNECT_ACTION, LOGOUT_ACTION];
          //   else actions = [RECONNECT_ACTION, LOGOUT_ACTION];

          // Array com botões de ações
          let actions;
          if (NAVIGATE_HOME_ACTION) actions = [NAVIGATE_HOME_ACTION, LOGIN_ACTION, LOGOUT_ACTION];
          else actions = [LOGIN_ACTION];

          // Get any associated status code
          const status = $scope.client.clientState.statusCode;

          // Connecting
          if (connectionState === ManagedClientState.ConnectionState.CONNECTING || connectionState === ManagedClientState.ConnectionState.WAITING) {
            $scope.status = {
        ...
        const canSubmitParameters = function canSubmitParameters(connectionState) {
          return connectionState === ManagedClientState.ConnectionState.WAITING || connectionState === ManagedClientState.ConnectionState.CONNECTED;
        };

        // Show status dialog when connection status changes
        $scope.$watchGroup(
          ["client.clientState.connectionState", "client.requiredParameters", "client.protocol", "client.forms"],
          function clientStateChanged(newValues) {
            ...
            if (requiredParameters && canSubmitParameters(connectionState)) notifyParametersRequired(requiredParameters);
        ...
        const preventDefaultDuringNotification = function preventDefaultDuringNotification(e) {
          if ($scope.status && $scope.client.clientProperties.focused) e.preventDefault();
        };
