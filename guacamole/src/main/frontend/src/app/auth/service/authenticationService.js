        ...
        requestService({
          ...
          [
            ...
            requestService.createErrorCallback(function authenticationFailed(error) {
              ...
              if (error.type === Error.Type.INVALID_CREDENTIALS) $rootScope.$broadcast("guacInvalidCredentials", parameters, error);
              // Request more credentials if provided credentials were not enough
              else if (error.type === Error.Type.INSUFFICIENT_CREDENTIALS) $rootScope.$broadcast("guacInsufficientCredentials", parameters, error);
